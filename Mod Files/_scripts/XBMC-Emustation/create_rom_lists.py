...
with open( xbmc.translatePath( "special://xbmc/system/" ) + "xbmc.log", "r" ) as XBMCLOG:
	for line in XBMCLOG:
		...
		if found:
			...
			Extensions			= [ "int","tap","z80","tzx","zip","bin","ccd","cue","j64","img","iso","rom","n64","z64","smd","smc","gb","gbc","gba","nes","sms","swc","gg","a26","a78","col","lnx","sfc","sg","fig","vms","exe" ]
			...
			if os.path.isfile( os.path.join( Emu_Path, "default.xbe" ) ):
				...
				if len(os.listdir( Roms_Folder )) > 0:
					...
					if Change_FBL_Rom_Path == 1:
						if os.path.isfile(Emu_Path + 'Path.ini'):
							for line in fileinput.input( Emu_Path + 'Path.ini', inplace=1):
								if 'ROMPath1=' in line:
									line = line = 'ROMPath1=' + Roms_Folder[:-1] + '\n'
								print line,
							Change_FBL_Rom_Path = 0
						else:
							with open( Emu_Path + 'Path.ini', "w") as outputfblfile:
								WriteFblFile = fbl_config % ( Roms_Folder[:-1] )
						...
						if os.path.isfile("E:\\TDATA\\4d414d46\\SYSTEM\\MAMEoX.ini"):
							for line in fileinput.input("E:\\TDATA\\4d414d46\\SYSTEM\\MAMEoX.ini", inplace=1):
								if 'RomsPath0 = ' in line:
									line = line = 'RomsPath0 = ' + Roms_Folder[:-1] + '\n'
								print line,
							Change_Mame_Rom_Path = 0
						else:
							if not os.path.isdir( "E:\\TDATA\\4d414d46\\SYSTEM" ):
								os.makedirs( "E:\\TDATA\\4d414d46\\SYSTEM" )
								if os.path.isfile( Emu_Path + 'general\\DRIVERS.list' ): shutil.copy2( Emu_Path + 'general\\DRIVERS.list', "E:\\TDATA\\4d414d46\\SYSTEM\\" )
							with open( "E:\\TDATA\\4d414d46\\SYSTEM\\MAMEoX.ini", "w") as outputmamefile:
								WriteMameFile = mame_config % ( Roms_Folder[:-1] )
					...
					for Items in sorted( os.listdir( Roms_Folder ) ):
						...
						if Items.endswith(tuple(Extensions)):
							...
							if CreateDialog == 1: 
								CreateDialog = 0
								pDialog.create( "Scanning for Roms","","Please wait..." )
							if CountList == 1 and Found_Roms == 0: pDialog.update( 0,"Processing [B][UPPERCASE]" + Emu_Name + "[/UPPERCASE][/B] Roms",Rom_Name_noext,"Please wait..." )
							log('|--------------------------------------------------------------------------------')
							...
							if Write_CUT_File:
								...
								with open( Rom_List_Path + Emu_Name + '.xml', "a") as outputmenufile:
									if Emu_Name == "fba":
										WriteMenuFile = menu_entry % (CountList,FBA_Rom_Name,Synopsis1,'RunScript( Special://xbmc/_scripts/XBMC-Emustation/launcher.py,' + Emu_XBE + ',' + Rom_Name_noext + ' )',"ActivateWindow(1101)",Synopsis2,TBN_File)
									elif Emu_Name == "mame":
										pass
									elif Emu_Name == "n64":
					...
					if Emu_Name == "atarijaguar.xml" or Emu_Name == "atarijaguarcd.xml":
						if os.path.isfile( os.path.join( Rom_List_Path + Emu_Name + '.xml' ) ): os.remove( Rom_List_Path + Emu_Name + '.xml' ) 
						if os.path.isfile( os.path.join( Rom_List_Path + Emu_Name + '.xml' ) ): os.remove( Rom_List_Path + Emu_Name + '_jump.xml' )	
					if Emu_Name == "mame":
						...
						if os.path.isfile( os.path.join( Rom_List_Path + Emu_Name + '.xml' ) ): os.remove( Rom_List_Path + Emu_Name + '.xml' )
						if os.path.isfile( os.path.join( Rom_List_Path + Emu_Name + '_jump.xml' ) ): os.remove( Rom_List_Path + Emu_Name + '_jump.xml' )
				else:
					pass
			log('|--------------------------------------------------------------------------------')
