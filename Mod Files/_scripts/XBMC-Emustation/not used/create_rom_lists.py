'''
	...
	This script can pars subdirectories but its slow in doing so and has some progress bar issues. ( displays the wrong % for example )
	So I don't use it.
		...
		for Emu_Path in sorted( os.listdir( Emulator_Path ) ):
			...
			if os.path.isfile( os.path.join( Emu_Path, "default.xbe" ) ):
				...
				if len(os.listdir( Roms_Folder )) > 0:
					...
					log('|--------------------------------------------------------------------------------')
					if Emu_Name == "fba":
						Rom_Count = sum([len(files) for root, dirs, files in os.walk(os.path.join( Roms_Path, "fba" ))])
					else:
						Rom_Count = sum([len(files) for root, dirs, files in os.walk(Roms_Folder)])
					for root, dirs, files in os.walk(Roms_Folder):
						Roms_Folder = root
						for Items in sorted( files ):
							Percent = (CountList * 100) / Rom_Count
							if os.path.isfile( os.path.join( root, 'rom set 1', Items ) ):
								Roms_Folder = os.path.join( root, 'rom set 1' )
								Items = 'rom set 1\\' + Items
							elif os.path.isfile( os.path.join( root, 'rom set 2', Items ) ):
								Roms_Folder = os.path.join( root, 'rom set 2' )
								Items = 'rom set 2\\' + Items
							elif os.path.isfile( os.path.join( root, 'rom set 3', Items ) ):
								Roms_Folder = os.path.join( root, 'rom set 3' )
								Items = 'rom set 3\\' + Items
							else:
								pass
							log('|--------------------------------------------------------------------------------')
							...
							if Items.endswith(tuple(Extensions)):
								...
								log('|--------------------------------------------------------------------------------')
								if Emu_Name == "genesis":
									Synopsis_Folder = os.path.join( Synopsis_Path, "megadrive" )
								elif Emu_Name == "famicom":
									Synopsis_Folder = os.path.join( Synopsis_Path, "nes" )
								elif Emu_Name == "tg16":
									Synopsis_Folder = os.path.join( Synopsis_Path, "pcengine" )
								elif Emu_Name == "tg-cd":
									Synopsis_Folder = os.path.join( Synopsis_Path, "pce-cd" )
								elif Emu_Name == "fba":
									Synopsis_Folder = os.path.join( Emulator_Path, "fba\\info\\emulation" )
									Synopsis_Ext = ".ini"
								else:
									Synopsis_Folder = os.path.join( Synopsis_Path, Emu_Name )
								if os.path.isdir( os.path.join( Synopsis_Folder, 'synopsis 1' ) ) or os.path.isdir( os.path.join( Synopsis_Folder, 'synopsis 2' ) ):
									if os.path.isfile( os.path.join( Synopsis_Folder, 'synopsis 1', Rom_Name_noext + '.txt' ) ):
										Synopsis_Sub_Folder = os.path.join( Synopsis_Folder,'synopsis 1' )
									elif os.path.isfile( os.path.join( Synopsis_Folder, 'synopsis 2', Rom_Name_noext + '.txt' ) ):
										Synopsis_Sub_Folder = os.path.join( Synopsis_Folder,'synopsis 2' )
									else:
										Synopsis_Sub_Folder = os.path.join( Synopsis_Folder )
								else:
									Synopsis_Sub_Folder = os.path.join( Synopsis_Folder )
								Synopsis_File = os.path.join( Synopsis_Sub_Folder, Rom_Name_noext + Synopsis_Ext )
								...
								if Write_List_File:
									...
									with open( Rom_List_Path + Emu_Name + '.xml', "a") as outputmenufile:
										if Emu_Name == "fba":
											pDialog.update( int( Percent ),"Creating [B][UPPERCASE]" + Emu_Name + "[/UPPERCASE][/B] Rom list",FBA_Rom_Name,"This can take some time, please be patient." )
											...
											pDialog.update( int( Percent ),"Creating [B][UPPERCASE]" + Emu_Name + "[/UPPERCASE][/B] Rom list",N64_Rom_Name,"This can take some time, please be patient." )
											...
											pDialog.update( int( Percent ),"Creating [B][UPPERCASE]" + Emu_Name + "[/UPPERCASE][/B] Rom list",Rom_Name_noext,"This can take some time, please be patient." )
