...
pDialog.update(0)
fav_data_with_sort = []
sort_s = sys.argv[1] if len(sys.argv) > 1 else 0
...
if os.path.isfile(Favourites_XML):
	try:
		...
		with open(Favourites_XML, 'r') as xml:
			content = xml.read().replace("'", '&apos;').replace('&', '&amp;')

		root = ET.parse(Favourites_XML).getroot()
		...
		for child in root:
			...
			fav_data = '    <favourite name="%s" thumb="%s">%s</favourite>' % (attrib_name, attrib_thumb, child_data)
