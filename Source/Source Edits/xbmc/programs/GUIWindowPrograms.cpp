	...
	if (item && !item->GetPropertyBOOL("pluginreplacecontextitems"))
	{
		...
		else
		{
			if (item->IsXBE() || item->IsShortCut())
			{
				...
				URIUtils::GetParentPath(item->GetPath(), patchedxbepath);
				patchedxbepath = CURL(patchedxbepath).GetWithoutUserDetails();
				CURL::Decode(patchedxbepath);
				if (CFile::Exists(patchedxbepath+"default_p.xbe"))
				{
					patchedxbepath = (patchedxbepath+"default_p.xbe");
				}
				if (CFile::Exists(patchedxbepath+"defaultxhd.xbe"))
				{
					patchedxbepath = (patchedxbepath+"defaultxhd.xbe");
				}
				if (CFile::Exists(patchedxbepath))
				{
					buttons.Add(CONTEXT_BUTTON_PATCHEDXBE, "Run Patched XBE");         // Patched XBE
				}  
...
bool CGUIWindowPrograms::OnContextButton(int itemNumber, CONTEXT_BUTTON button)
{
	...
	case CONTEXT_BUTTON_PATCHEDXBE:
		{
			CBuiltins::Execute("runxbe("+patchedxbepath+")");
			return true;
		}
		...
		if (item->IsXBE())
		{
			...
			if (!dwTitleID)
			{
				...
				if (CUtil::GetXBEDescription(item->GetPath(), description) && (!item->IsLabelPreformated() && !item->GetLabel().IsEmpty()))
				{
					...
					if (!g_guiSettings.GetBool("mygames.slowgameparsing"))
					{
						CLog::Log(LOGDEBUG,"Added to database: %s",description.c_str());
					}
				}
